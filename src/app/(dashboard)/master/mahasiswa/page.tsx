...
import {
    Search, Plus, MoreHorizontal, Filter, ArrowUpDown, ChevronLeft, ChevronRight, User, CheckCircle2, XCircle, MoreVertical, Download, Trash2, Edit, Trash, Clock, Archive, RotateCcw, Upload
} from "lucide-react"
...
import { MahasiswaFormDialog } from "@/components/master/mahasiswa/form-dialog"
import { MahasiswaImportDialog } from "@/components/master/mahasiswa/import-dialog"
import { Mahasiswa, ApiResponse } from "@/types/master"
...
export default function MahasiswaPage() {
    ...
    const [meta, setMeta] = useState<Omit<ApiResponse<Mahasiswa>, 'data'> | null>(null)

    const [isDialogOpen, setIsDialogOpen] = useState(false)
    const [isImportDialogOpen, setIsImportDialogOpen] = useState(false)
    const [selectedData, setSelectedData] = useState<Mahasiswa | null>(null)
    ...
    const [dataMode, setDataMode] = useState<number>(1) // 1 = Aktif, 2 = Terhapus
    ...
    const fetchData = async () => {
        ...
        const res = await getMahasiswa(urlPage, search, statusParam)
    ...
    const openEditDialog = (item: Mahasiswa) => {
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight mb-1">
                        {dataMode === 1 ? "Master Mahasiswa" : "Sampah Mahasiswa"}
                    </h1>
                    <p className="text-muted-foreground">
                        {dataMode === 1
                            ? "Kelola data mahasiswa aktif."
                            : "Data mahasiswa yang sudah dihapus sementara."}
                    </p>
                </div>
              ...
              ${dataMode === 1
                    ...
                    <button
                        ...
                        className="hidden sm:flex items-center gap-2 h-8 px-3 bg-secondary text-secondary-foreground rounded-md hover:opacity-90 transition-all text-xs font-medium border border-border"
                    >
                    ...
                    <table className="w-full caption-bottom text-sm text-left">
                        <thead className="[&_tr]:border-b border-border">
                            <tr className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                                ...
                                <th className="shadcn-table-header">NIM</th>
                                ...
                                <th className="shadcn-table-header">L/P</th>
                                <th className="shadcn-table-header">Kontak</th>
                            ...
                            {loading ? (
                                ...
                                data.map((item, index) => (
                                    ...
                                    >
                                        ...
                                        <td className="shadcn-table-cell text-center">
                                            <div className="flex justify-center gap-2">
                                                    ...
                                                    <>
                                                        ...
                                                        <DropdownMenu>
                                                            ...
                                                            <DropdownMenuContent align="start">
                                                                <DropdownMenuItem
                                                                    onClick={() => openConfirmDialog(item.id_mahasiswa, 'soft')}
                                                                    className="text-red-600 focus:text-red-600 cursor-pointer"
                                                                >
                                                                    <Trash2 className="mr-2 h-4 w-4" /> Hapus
                                                                </DropdownMenuItem>
                                                                <DropdownMenuItem
                                                                    onClick={() => openConfirmDialog(item.id_mahasiswa, 'hard')}
                                                ...
                                                {dataMode === 2 && (
                                                    <>
                                                        <button
                                                            onClick={() => openConfirmDialog(item.id_mahasiswa, 'restore')}
                                        ...
                                        <td className="shadcn-table-cell font-mono text-xs font-medium text-foreground">
                                            {item.nim}
                                        </td>
                                        ...
                                        <td className="shadcn-table-cell font-medium">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-sm">{item.nama_mahasiswa}</span>
                                            </div>
                                        </td>
                                        ...
                                        <td className="shadcn-table-cell text-sm text-muted-foreground">
